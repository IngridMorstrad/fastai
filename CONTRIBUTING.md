...
Use short, descriptive commit messages prefixed with a type:
...
* `chore:` for maintenance tasks (CI config, dependency updates, etc.)

Example: `fix: reset samp_idx in CorpusBLEUMetric.reset to avoid stale state`

Keep the subject line under 72 characters. If more context is needed, add a blank line followed by a longer description in the commit body.

## Common Pitfalls for New Contributors

1. **Editing generated files directly.** Files marked `AUTOGENERATED! DO NOT EDIT!` are exported from notebooks in `/nbs`. Edit the source notebook instead, then run `nbdev_export`.
2. **Forgetting to run `nbdev_install_hooks`.** Without git hooks installed, notebook metadata will pollute diffs and cause merge conflicts.
3. **Using PEP 8 formatting.** This project intentionally uses a compact coding style optimized for numerical and interactive programming. Read the [style guide](https://docs.fast.ai/dev/style.html) before reformatting code.
4. **Mixing functional changes with style changes.** Keep PRs focused on one concern. Style-only and logic-change commits should be in separate PRs.
5. **Not running `nbdev_export` after notebook changes.** If you modify an exported cell in a notebook, the corresponding `.py` file must be regenerated before committing.

## Understanding the nbdev Workflow

The library source code lives in Jupyter notebooks under `/nbs`. The Python files in `/fastai` are generated from those notebooks by `nbdev_export`. This means:

* **To change library code:** edit the relevant notebook, then run `nbdev_export` to regenerate the `.py` files.
* **To change library code and update the notebook:** if you edited a `.py` file directly during experimentation, run `nbdev_update` to sync changes back to notebooks before committing.
* **To run the test suite:** use `nbdev_test` which executes notebook cells flagged for testing in parallel.
* **To build docs locally:** run `nbdev_docs` (requires quarto) to preview documentation generated from notebooks.

A typical contribution cycle looks like:

```
1. Edit the notebook in /nbs
2. Run nbdev_export        (generates .py files)
3. Run tests               (nbdev_test or pytest tests/)
4. Commit both .ipynb and .py changes together
```
