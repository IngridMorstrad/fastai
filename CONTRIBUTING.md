...
## Navigating the Notebook-to-Source Pipeline

fastai uses [nbdev](https://nbdev.fast.ai/) to generate library source from Jupyter notebooks. This means the `.py` files under `fastai/` are generated artifacts. Understanding this pipeline is essential for making changes correctly.

### How the pipeline works

1. **Notebooks are the source of truth.** All exported code lives in cells tagged with `#|export` in the `/nbs` directory.
2. **`nbdev_export`** reads notebooks and writes the corresponding `.py` files into `fastai/`.
3. **`nbdev_update`** goes the other direction: if you edit a `.py` file directly, it syncs those changes back into the notebook.
4. **CI enforces sync.** The GitHub Actions workflow runs `python -m nbdev.export` and fails if any `.py` files differ from what the notebooks produce.

### Common mistakes and how to avoid them

| Mistake | What happens | Fix |
|---------|-------------|-----|
| Editing a `.py` file without running `nbdev_update` | CI fails because notebook and source are out of sync | Run `nbdev_update` after editing `.py` files, then verify the notebook diff looks correct |
| Adding a new function in a notebook without `#|export` | Function exists in the notebook but never appears in the library | Add `#|export` to the cell |
| Editing a file with the `# AUTOGENERATED` header by hand | Your edits are overwritten next time anyone runs `nbdev_export` | Edit the source notebook instead |
| Running `nbdev_export` with uncommitted notebook changes | Unexpected diffs in generated files | Always commit or stash notebook changes before exporting to keep diffs reviewable |

### Practical workflow

```bash
# After modifying a notebook:
nbdev_export              # regenerate .py files
git diff                  # verify only expected files changed
nbdev_test --fname nbs/00_torch_core.ipynb  # test just your notebook

# After modifying a .py file directly:
nbdev_update              # sync change back to notebooks
nbdev_export              # round-trip to confirm consistency
git diff                  # should show no unexpected changes
```

### When to edit notebooks vs. source files

- **New features, API changes, docstrings**: always edit the notebook. The narrative structure helps reviewers understand your intent.
- **Quick bug fixes in internal helpers**: editing the `.py` file directly and running `nbdev_update` is sometimes faster, but always verify with a round-trip (`nbdev_update` then `nbdev_export` then `git diff`).
- **Tests**: test files under `tests/` are plain Python and are not generated by nbdev. Edit them directly.
