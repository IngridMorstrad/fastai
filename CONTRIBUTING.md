...
## Code Quality: Finding and Removing Dead Code

Keeping the codebase lean improves readability and reduces maintenance burden. Here is how to identify dead code in this project:

### What counts as dead code

- **Unused imports** - modules imported at the top of a file but never referenced in the body
- **Unused functions/classes** - definitions that are never called or instantiated anywhere
- **Dead test fixtures** - pytest fixtures defined in `conftest.py` or test files but never requested by any test function
- **Orphaned scripts** - standalone `.py` files that are not imported, executed, or referenced from documentation

### How to find dead code safely

1. **Unused imports**: Run a static analysis tool or manually check with grep:
   ```bash
   # Check if 'some_module' is used beyond its import line
   grep -n "some_module" path/to/file.py
   ```

2. **Unused functions**: Search for all call sites of a function across the repo:
   ```bash
   grep -r "function_name" --include="*.py" --include="*.ipynb" .
   ```

3. **Dead test fixtures**: Search for fixture usage across all test files:
   ```bash
...
- **Never modify auto-generated files** under `fastai/` that are produced by `nbdev_export`. These files contain the comment `# AUTOGENERATED! DO NOT EDIT!` at the top. To fix dead code in these files, edit the source notebook in `nbs/` and re-export.
- **Test files and scripts** (`tests/`, `nbs/examples/`, `setup.py`) are safe to modify directly since they are not auto-generated.
- **Always verify** that removing code does not break any tests by running `pytest` on the affected test files.
- **Update `REPO_LINE_SIZE.md`** after removing dead code to keep the line count accurate. Use:
  ```bash
  find . -name "*.py" -not -path "*/__pycache__/*" | sort | xargs wc -l
  ```
