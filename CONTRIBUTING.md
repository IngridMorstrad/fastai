...
## Working with the nbdev Export System

The fastai library uses [nbdev](https://nbdev.fast.ai/) to maintain a single source of truth in Jupyter notebooks. Understanding the export system is essential for productive contributions.

### How the export pipeline works

1. **Notebooks are the source** - All library code lives in `/nbs/*.ipynb`. Python files in `fastai/` are auto-generated.
2. **Export markers** - Cells tagged with `#|export` are exported to the corresponding `.py` module. The mapping is declared in a `#|default_exp` cell at the top of each notebook.
3. **`nbdev_export`** reads notebooks and writes `.py` files. Run this after editing notebook cells.
4. **`nbdev_update`** does the reverse: if you patch a `.py` file directly (e.g., a quick bug fix), this propagates changes back into the notebooks.

### Common workflows

| Task | Command |
|------|---------|
| Edit code in a notebook, sync to library | `nbdev_export` |
| Edit `.py` directly, sync back to notebook | `nbdev_update` |
| Check nothing is out of sync | `nbdev_export && git diff` (should show no changes) |
| Run all notebook tests | `nbdev_test` |
| Run tests for a single notebook | `nbdev_test --fname nbs/00_torch_core.ipynb` |
| Preview docs locally | `nbdev_preview` |

### Tips for avoiding common pitfalls

- **Never hand-edit auto-generated files** - the `AUTOGENERATED! DO NOT EDIT!` header means the file is overwritten by `nbdev_export`. Your edits will be lost.
- **Keep cell outputs clean** - run `nbdev_install_hooks` once after cloning so git hooks strip cell outputs automatically, preventing merge conflicts from output metadata.
- **Test before and after export** - a cell can pass interactively in the notebook but fail once exported if it relies on notebook-scoped imports or variables from non-exported cells.
- **Mind the `__all__` list** - the `# %% auto 0` cell auto-generates `__all__` from exported symbols. If your new function does not appear, verify it has an `#|export` tag and that `nbdev_export` was re-run.
- **Use `#|hide`** - for helper cells that should run in the notebook (e.g., test assertions, plotting examples) but must not be exported or appear in docs.
- **`show_doc` cells** - adding `show_doc(YourClass.method)` in the notebook generates API documentation. Include one for each public method you add.

### Verifying sync in CI

The CI workflow runs a sync check equivalent to:
...
If this fails, it means the `.py` files in your PR do not match what `nbdev_export` would produce. Re-run `nbdev_export` locally and commit the result.
