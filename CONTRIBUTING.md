...
Understanding how the repository is organized helps you find the right place to make changes and avoid accidentally breaking the nbdev sync pipeline.

### Directory Layout

```
fastai/
├── nbs/                  # Source of truth: Jupyter notebooks that generate the library
│   ├── 00_torch_core.ipynb
│   ├── 01_layers.ipynb
│   ├── ...
│   └── examples/         # Example notebooks (not exported to library code)
├── fastai/               # Auto-generated Python modules (via nbdev_export from nbs/)
│   ├── torch_core.py     # Tensor utilities, type coercion, GPU helpers
│   ├── data/             # DataLoaders, Datasets, core data pipeline transforms
│   ├── text/             # NLP tokenizers, numericalizers, language model data
│   ├── vision/           # Image augmentations, learners, model architectures
│   ├── tabular/          # Tabular data processing and models
│   ├── medical/          # Medical imaging utilities
│   ├── callback/         # Training loop callbacks (logging, scheduling, mixed precision)
│   └── test_utils.py     # Shared test helpers and installation diagnostics
├── tests/                # Unit and integration tests (run via pytest)
├── .github/workflows/    # CI definitions (nbdev sync check on PRs)
└── settings.ini          # nbdev project configuration (version, requirements, etc.)
```
...
|---------|----------|---------|
| **Transform** | `fastai/core.py`, `fastai/data/` | Reversible, type-dispatched data transformations |
| **Pipeline** | `fastai/data/core.py` | Ordered composition of Transforms |
| **DataLoaders** | `fastai/data/core.py` | Train/valid DataLoader pair with shared transforms |
| **Learner** | `fastai/learner.py` | Wraps model + data + optimizer + loss into a trainable unit |
| **Callback** | `fastai/callback/core.py` | Hooks into Learner events (before/after batch, epoch, fit) |
| **Metric** | `fastai/metrics.py` | Accumulates predictions per epoch and computes a score |

### Where to Make Changes

- **Library logic**: edit the corresponding notebook in `nbs/`, then run `nbdev_export` to regenerate the `.py` file.
- **Tests**: edit files in `tests/` directly; these are not auto-generated.
- **Documentation prose**: edit notebooks in `nbs/`; markdown cells become the docs site.
- **CI / repo tooling**: edit files in `.github/`, `settings.ini`, or project root directly.

### Avoiding the Most Common Mistake

Files under `fastai/` that begin with the comment `# AUTOGENERATED! DO NOT EDIT!` are produced by nbdev from the notebooks. If you edit them directly, CI will fail the sync check because the notebook and `.py` file will be out of sync. Always trace back to the notebook source (the comment header tells you which notebook) and make your edit there.
