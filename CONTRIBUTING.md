...
## Dead Code Detection and Code Hygiene

Unused imports and dead functions add noise and increase cognitive load for readers. When you notice dead code during development, please remove it as part of your PR (or submit a separate cleanup PR).

### How to identify dead code

1. **Unused imports** - Look for imports that are never referenced in the file. Tools like `pyflakes` or `ruff check --select F401` can detect these automatically.
2. **Dead functions** - Functions that are defined but never called anywhere in the codebase. Use `grep -r "function_name"` across the repo to confirm zero callers.
3. **Unreachable code** - Code after unconditional `return`, `raise`, `break`, or `continue` statements.

### Important caveats

- **Auto-generated files**: Files in `fastai/` marked with `# AUTOGENERATED! DO NOT EDIT!` are generated from notebooks in `nbs/`. If dead code exists in these files, the fix must be made in the corresponding notebook, then re-exported with `nbdev_export`. Editing the `.py` file directly will be overwritten.
- **Re-exported symbols**: Some imports in `__init__.py` or `all.py` files exist solely to make symbols available to downstream users via `from fastai.x import *`. These are not dead code even if unused within the file itself.
- **Test fixtures and conftest**: Imports in test files that appear unused may be pytest fixtures injected via `conftest.py` or parameterized test helpers. Verify before removing.

### Tracking removals

When you remove dead code, update `REPO_LINE_SIZE.md` to document what was removed and refresh the line count. This helps maintainers track code health over time.
