...
Dead code increases maintenance burden, clutters test output, and makes the codebase harder to navigate. Here are tips for detecting and safely removing it:

### Unused Imports

1. **Search for imports that have no other reference in the file.** A quick heuristic: if `import foo` appears but `foo` is never referenced after the import line, it is dead.
2. **Common offenders in test files**: `import pytest` when no `pytest.raises`, `pytest.mark`, or fixtures are used; `import math` when no `math.*` calls exist; `from unittest.mock import patch` when only `MagicMock` is used.
3. **Verify before removing**: grep the file for every name the import brings in. Some imports have side effects (e.g., registering plugins), but this is rare in test files.

### Duplicated Test Infrastructure

1. **Look for identical helper classes or setup blocks** across multiple test files. If two or more files share the same mock setup, extract it into a shared module (e.g., `tests/tracker_test_utils.py`).
2. **Keep test-specific helpers local.** Only extract code that is truly identical. If a helper has test-specific logic (custom `state_dict` behavior, unique fixture data), leave it in the test file.
3. **Import shared utilities with a simple `from module import ...`** so each test file remains self-contained and readable.

### Validation Workflow

1. After removing dead code, verify every modified file still parses: `python3 -c "import ast; ast.parse(open('path/to/file.py').read())"`.
2. Run the test suite (`pytest tests/`) to confirm nothing breaks.
3. Never modify files under `fastai/` that contain an `AUTOGENERATED` header in the first few lines. Edit the source notebook instead.
4. Update `REPO_LINE_SIZE.md` with fresh `wc -l` output after your changes.
