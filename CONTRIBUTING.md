...
## Common Pitfalls for nbdev Contributors

Working with nbdev can be surprising if you are used to a traditional Python project layout. Here are mistakes that trip up new contributors most often:

1. **Editing generated `.py` files directly** - The source of truth is the notebooks in `/nbs`. If you edit a file under `fastai/` that was generated by `nbdev_export`, your changes will be silently overwritten the next time someone runs `nbdev_export`. Always edit the notebook cell with the `#|export` directive and then run `nbdev_export` to regenerate the library file.

2. **Forgetting `nbdev_install_hooks`** - Without the git hooks installed, notebooks retain runtime metadata (cell execution counts, widget state) that produces massive, unreadable diffs. Run `nbdev_install_hooks` once after cloning to keep commits clean.

3. **Running notebooks out of order** - Some notebooks depend on cells executed in earlier notebooks (shared utility functions, data downloads). If `nbdev_test` fails on a single notebook, try running the full test suite first to ensure prerequisites have been executed.

4. **Mismatched cell tags** - A cell that should be exported must start with `#|export` (not `# export` or `#export`). A missing pipe character causes the function to appear in docs but never reach the library `.py` file, leading to `ImportError` at runtime.

5. **Committing large notebook outputs** - Visualizations, training logs, and DataFrame renders inflate the repo and slow down cloning. Clear outputs before committing or rely on `nbdev_install_hooks` to strip them automatically.
