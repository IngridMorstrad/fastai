...
Keeping the codebase free of unused imports and unreachable functions reduces cognitive load for readers and prevents import-time side effects. Here is a workflow for finding and safely removing dead code:

### 1. Scan for unused imports with AST analysis
...
Run this against test files and standalone scripts to find imports that were added during development but never used.

### 2. Know which files are safe to edit

- **Safe targets**: `tests/`, `setup.py`, `dev_nbs/`, `nbs/examples/` -- these are not auto-generated.
- **Unsafe targets**: files under `fastai/` that have nbdev markers (`# AUTOGENERATED`, `#|export`). Editing these directly breaks the `nbdev_export` sync check in CI. Edit the source notebook instead.

### 3. Verify your removals

After removing dead code:
1. Run `python3 -c "import ast; ast.parse(open('file.py').read())"` to confirm valid syntax.
2. Run `python3 -m pytest <test_file> -x -q` to confirm nothing depends on the removed import at runtime.
3. Update `REPO_LINE_SIZE.md` with the new `wc -l` output so the repository tracks its size over time.

### 4. Common patterns that indicate dead code

- Imports added for debugging (e.g., `import pdb`, `from pprint import pprint`) left behind after the session.
- Imports copied from another file as boilerplate but never referenced.
- Helper functions defined for a feature that was later rewritten.
- Conditional branches guarded by constants that are always True/False.
