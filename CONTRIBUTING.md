...
Jupyter notebooks are JSON files, which makes merge conflicts particularly difficult to resolve with standard text-based tools. Here is how to handle them safely in this project.

### Why notebook conflicts are hard

A `.ipynb` file stores cell outputs (images, tracebacks, HTML tables), execution counts, and metadata alongside your code. Two contributors can create a conflict simply by running the same notebook on different data, even without changing any code. Standard `git diff` output on JSON is nearly unreadable, and a single misplaced comma or bracket will corrupt the file.

### Prevention (strongly preferred)

1. **Always run `nbdev_install_hooks`** after cloning. This installs a git filter that strips outputs and metadata on commit, eliminating the most common source of spurious conflicts.
2. **Clear all outputs** before committing if you are not using nbdev hooks: _Kernel > Restart & Clear Output_ in Jupyter, or run `nbdev_clean --fname nbs/your_notebook.ipynb` from the terminal.
3. **Keep PRs small and focused.** A PR that touches many notebooks greatly increases the chance of conflicts with other in-flight work.
4. **Rebase often.** Pull changes from `master` frequently so you catch conflicts early when they are small.

### Resolution workflow

When `git merge` or `git rebase` reports a conflict in a `.ipynb` file:

1. **Do not hand-edit the JSON.** Open it in a text editor only to understand which cells differ; never try to manually reconcile the JSON structure.

2. **Use `nbdev_fix_merge`** (preferred):
   ```bash
   nbdev_fix_merge nbs/conflicted_notebook.ipynb
   ```
   This tool attempts to automatically resolve cell-level conflicts by keeping both versions of conflicting cells and marking them for your review. After running it, open the notebook in Jupyter and manually verify correctness.

3. **If `nbdev_fix_merge` cannot resolve it**, take an "ours then theirs" approach:
   ```bash
   # Accept the upstream (master) version of the notebook
   git checkout --theirs nbs/conflicted_notebook.ipynb
   # Re-apply your changes manually in Jupyter
   ```
   This is safer than attempting to merge two divergent JSON trees by hand.

4. **After resolving, always run:**
   ```bash
   nbdev_export          # regenerate .py files from the fixed notebook
   nbdev_clean           # strip outputs/metadata
   nbdev_test --fname nbs/conflicted_notebook.ipynb  # verify it runs
   ```

5. **Stage and continue** the merge/rebase:
   ```bash
   git add nbs/conflicted_notebook.ipynb
...
| Tool | Purpose |
|------|---------|
| `nbdev_fix_merge` | Automatically resolves notebook merge conflicts at the cell level |
| `nbdev_clean` | Strips outputs and metadata so diffs only show real code changes |
| `nbdime` (optional) | Provides notebook-aware diff and merge commands (`nbdiff`, `nbmerge`) that understand cell structure |
| `nbdev_install_hooks` | Installs git hooks that auto-clean notebooks, preventing most conflicts before they happen |

### When to ask for help

If a conflict involves more than 3-4 cells of actual code changes (not just output diffs), it may be faster to close your PR, pull the latest `master`, and re-apply your changes from scratch on a clean copy of the notebook. This avoids the risk of subtle JSON corruption that passes `git status` but breaks Jupyter.
