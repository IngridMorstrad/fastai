...
## Identifying Orphaned (Non-nbdev) Code

This project uses [nbdev](https://nbdev.fast.ai/) for notebook-driven development. All proper library files under `fastai/` are auto-generated from notebooks and contain the header comment `# AUTOGENERATED! DO NOT EDIT!` at the top of the file. Occasionally, automated tools or incomplete PRs introduce Python files that bypass this workflow. These orphaned files become dead code that is never imported or used by the rest of the library.

To determine if a file under `fastai/` is orphaned, check for these three signs:

1. **Missing AUTOGENERATED header**: Open the file and look for `# AUTOGENERATED! DO NOT EDIT!` in the first few lines. If it is absent, the file was not generated by nbdev from a notebook.
2. **Not listed in `fastai/_modidx.py`**: The module index tracks all nbdev-managed modules. Search `_modidx.py` for the module path (e.g., `fastai.vision.gradcam`). If it does not appear, nbdev does not know about the file.
3. **Not imported by the relevant `all.py` or `__init__.py`**: Each subpackage (e.g., `fastai/vision/all.py`) imports its public API. If the file is not imported there either, it has no path to being loaded at runtime.

A file that fails all three checks is almost certainly orphaned code. It was likely added by an automated tool or an incomplete contribution that was never wired into the nbdev pipeline. Such files are safe to remove, along with any dedicated test files that only exercise the orphaned module.

When removing orphaned code:

- Delete the orphaned `.py` file from `fastai/`
- Delete its corresponding test file from `tests/` (if one exists)
- Run `grep -r <module_name> fastai/ tests/ --include='*.py'` to confirm no remaining references
- Update `REPO_LINE_SIZE.md` with fresh `wc -l` output
