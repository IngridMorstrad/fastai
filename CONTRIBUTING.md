...
# 4. Install nbdev hooks (required for notebook PRs)
nbdev_install_hooks
...
This project uses **pytest** for its test suite. Tests live in the `tests/` directory.
...
python -m pytest tests/test_data_load.py -v

# Run a specific test class or function
python -m pytest tests/test_data_load.py::TestFaCollate -v

# Run tests with shorter output
python -m pytest tests/ -q
```

**Prerequisites**: Make sure you have installed the project in development mode before running tests:

```bash
pip install -e ".[dev]"
```

Some tests require PyTorch and other deep learning dependencies. If a test file fails to import, check that you have the required packages installed (torch, numpy, etc.).

### Test Organization

Each test file in `tests/` covers a specific module:

| Test File | Module Tested |
|-----------|---------------|
| `test_data_load.py` | `fastai/data/load.py` (DataLoader, fa_collate, fa_convert) |
| `test_data_transforms.py` | `fastai/data/transforms.py` |
| `test_layers.py` | `fastai/layers.py` |
| `test_losses.py` | `fastai/losses.py` |
| `test_metrics.py` | `fastai/metrics.py` |
| `test_optimizer.py` | `fastai/optimizer.py` |
| `test_torch_core.py` | `fastai/torch_core.py` |
| `test_text_core.py` | `fastai/text/core.py` |

When adding tests, place them in the file that corresponds to the module being tested. If no file exists yet, create a new `tests/test_<module_name>.py` file following the same class-based structure used in existing test files.
...
### Important: Autogenerated Files

Most Python files under `fastai/` are **autogenerated from notebooks** via `nbdev_export`. These files contain a header comment:

```python
# AUTOGENERATED! DO NOT EDIT!
```

**Never edit these files directly.** Instead, edit the corresponding notebook in `nbs/` and run `nbdev_export`. The CI workflow verifies that library files are in sync with notebooks, and your PR will fail if you hand-edit an autogenerated file.

Files that are safe to edit directly (not autogenerated):
