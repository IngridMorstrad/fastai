...
The fastai library uses [nbdev](https://nbdev.fast.ai/) for literate programming. Here is how the build pipeline works:

### Notebooks are the source of truth

The Jupyter notebooks in the `nbs/` directory are the canonical source for most of the library code. Each notebook corresponds to a module in the `fastai/` package. For example, `nbs/00_torch_core.ipynb` generates `fastai/torch_core.py`.

### Autogenerated vs. non-autogenerated files

- **Autogenerated files** have a header comment `# AUTOGENERATED! DO NOT EDIT!` and contain cell markers like `# %% ../nbs/XX_name.ipynb N`. These files are generated by running `nbdev_export` and must NOT be edited directly. To change code in these files, edit the corresponding notebook cell (look for cells beginning with `#|export`) and then re-export.
- **Non-autogenerated files** can be edited directly. These include:
  - `fastai/imports.py`
  - `fastai/fp16_utils.py`
  - `fastai/torch_imports.py`
  - `fastai/torch_basics.py`

### Export and sync workflow

After editing a notebook cell marked with `#|export`:

1. Run `python3 -m nbdev.export` (or `nbdev_export`) to regenerate the corresponding `.py` files.
2. Verify sync by running `git status --porcelain -uno`. If this shows no output after exporting, your notebooks and library code are in sync.
3. The CI pipeline runs exactly this check: it exports and then verifies no files have changed.

### Useful commands

| Command | Purpose |
|---------|---------|
| `nbdev_export` | Export notebook cells to `.py` modules |
| `nbdev_update` | Sync changes from `.py` files back into notebooks (if you edited a non-autogenerated `.py`) |
| `nbdev_test` | Run tests defined in notebooks |
| `nbdev_install_hooks` | Install git hooks that clean notebook metadata before commits |

### Tips for contributors

- Always check if a `.py` file is autogenerated before editing it. Open the file and look for the `AUTOGENERATED` header.
- If you are unsure which notebook corresponds to a `.py` module, check the cell markers in the `.py` file (e.g., `# %% ../nbs/00_torch_core.ipynb 5` means cell 5 of that notebook).
- The `_modidx.py` file is also generated and should not be hand-edited.
