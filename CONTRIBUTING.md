...
## Navigating and Debugging the fastai Source Code

Working with the fastai source can be disorienting because of the heavy use of metaprogramming, callbacks, and type dispatch. This section provides practical techniques for tracing execution flow and diagnosing issues.

### Understanding the Callback Order

Callbacks run in a defined sequence controlled by the `order` attribute. When debugging unexpected behavior during training:

1. Print the active callbacks and their order: `sorted(learn.cbs, key=lambda c: c.order)`
2. Identify which callbacks touch the event you are investigating (e.g., `after_loss`, `before_step`)
3. Remember that `CancelBatchException`, `CancelTrainException`, etc. are control flow, not errors. A callback raising one of these is intentionally short-circuiting the loop.

### Tracing Type Dispatch

fastai uses `typedispatch` (from fastcore) to select function implementations based on argument types at runtime. If you get unexpected behavior from a dispatched function like `show_batch` or `show_results`:

1. List all registered implementations: `show_batch.funcs` (or the dispatched function's `.funcs` attribute)
2. Check which implementation will be selected for your types: `show_batch.dispatch(type(x))` where `x` is the first positional argument
3. If your custom type is not matching, verify the MRO with `type(x).__mro__` and confirm the dispatch annotation matches a class in that chain

### Reading Training Loop Errors

Errors during `learn.fit()` can be hard to locate because the traceback traverses the callback machinery. Key tips:

- The real exception is usually the innermost frame. Scroll past the `Callback.__call__` and `Learner._do_epoch_*` wrappers.
- If the traceback mentions `CancelFitException` or `CancelEpochException`, a callback intentionally stopped training (e.g., `EarlyStoppingCallback`, `TerminateOnNaNCallback`). Check the callback's `after_epoch` logic.
- `AttributeError: 'Learner' object has no attribute '...'` often means a callback expected another callback to have run first. Check the `order` attributes.

### Using `Learner.summary` for Shape Debugging

When model forward pass errors occur (shape mismatches, wrong number of channels):

1. Call `learn.summary()` to see the output shape at each layer
2. Note that `summary` uses hooks, so layers wrapped in custom modules may not appear unless they register sub-hooks
3. For manual inspection, temporarily add `Callback` with `def after_pred(self): print(self.pred.shape)` to see the live prediction shape

### Inspecting the Recorder

The `Recorder` callback (always active by default) stores training state:

- `learn.recorder.lrs` - learning rates per batch
- `learn.recorder.losses` - raw training losses per batch
- `learn.recorder.values` - per-epoch metrics (aligned with `learn.recorder.metric_names[1:]`)

This data is useful for diagnosing vanishing/exploding gradients or verifying that schedulers are applying the expected LR curve.

### Common Source Layout Patterns

- Files under `fastai/` are **auto-generated** from notebooks in `nbs/`. If you see `# AUTOGENERATED! DO NOT EDIT!`, edit the corresponding notebook instead.
- `store_attr()` (from fastcore) saves constructor arguments as instance attributes. If a method references `self.foo` but you do not see it assigned, check the `__init__` for a `store_attr()` call.
- `@patch` adds methods to existing classes from outside the class definition. If a method is not in the class body, search for `@patch` decorators targeting that class.
- `@delegates` copies parameter signatures from one function to another. The actual accepted kwargs may not be visible in the decorated function's signature.
