...
- **Total lines**: 25758
- **Dead code removed this pass**: 12 lines (unused imports across test files and fp16_utils.py)
...
| File | Removed |
|------|---------|
| `fastai/fp16_utils.py` | Unused import `torch.nn as nn` (code uses `torch.nn` directly, never the `nn` alias) |
| `tests/test_layers.py` | Unused imports `pytest` and `numpy as np` |
| `tests/test_losses.py` | Unused imports `pytest`, `numpy as np`, and `torch.nn.functional as F` |
| `tests/test_metrics.py` | Unused imports `pytest`, `torch.nn.functional as F`, and `optim_metric` |
| `tests/test_optimizer.py` | Unused imports `pytest`, `math`, `rms_prop_step`, `adam_step`, and `OptimWrapper` |
