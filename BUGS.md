...
- `Learner.summary` does not count parameters for `ParameterModule` instances whose `forward` is never invoked during the model forward pass, so hooks in `layer_info` never fire for them (`fastai/callback/hook.py`)
