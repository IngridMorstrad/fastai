...
- `Learner.summary` did not count parameters for individual `ParameterModule` instances because `layer_info` checked `hasattr(m, 'weight')` which is False for ParameterModule (`fastai/callback/hook.py`) - fixed by using `has_params(m)` instead
