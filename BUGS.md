...
- `SentencePieceTokenizer` does not forward special token symbols to the underlying SentencePiece model (`fastai/text/core.py`) - already handled via `--user_defined_symbols` in the `train` method; the stale `#TODO` comment was misleading
