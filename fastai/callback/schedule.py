...
@patch
def fit_warmup(self:Learner, n_epoch, lr=None, warmup_pct=0.25, warmup_mode='linear', div_final=1e5, wd=None,
               cbs=None, reset_opt=False, start_epoch=0):
    "Fit with a linear or exponential LR warmup from near-zero to `lr` over `warmup_pct` of training, then cosine decay."
    if self.opt is None: self.create_opt()
    self.opt.set_hyper('lr', self.lr if lr is None else lr)
    lr = np.array([h['lr'] for h in self.opt.hypers])
    sched_fn = SchedExp if warmup_mode == 'exp' else SchedLin
    scheds = {'lr': combine_scheds([warmup_pct, 1-warmup_pct], [sched_fn(lr/100, lr), SchedCos(lr, lr/div_final)])}
    self.fit(n_epoch, cbs=ParamScheduler(scheds)+L(cbs), reset_opt=reset_opt, wd=wd, start_epoch=start_epoch)
